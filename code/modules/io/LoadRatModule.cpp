...
void LoadRatModule::run( DataManager& data) const
{
  ...
  switch(dim){
	  ...
	  data.setOutputData("img",new Matrix(img));
	  data.setOutputData("real",new Matrix(real));
	  data.setOutputData("imag",new Matrix(imag));
}

MetaData LoadRatModule::getMetaData() const
{
	map<string, DataDescription> output = {
		{"img", DataDescription(MATRIX, "the loaded image.") },
		{"imag", DataDescription(MATRIX, "the loaded image.") },
		{"real", DataDescription(MATRIX, "the loaded image.") }
	};
